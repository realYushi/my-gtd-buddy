...
```bash
./scripts/reminders.sh today
./scripts/reminders.sh counts
./scripts/calendar.sh free
...
```bash
./scripts/reminders.sh context @home
```
...
```bash
./scripts/state.sh session coach 1
./scripts/state.sh velocity <items_if_any>
```
